...
module Puppet::Util::Windows::Process
  ...
  def open_process_token(handle, desired_access)
    token_handle = FFI::MemoryPointer.new(:uint, 1)
    result = API.open_process_token(handle, desired_access, token_handle)

    return token_handle.read_uint unless !result
    raise Puppet::Util::Windows::Error.new(
      "OpenProcessToken(#{handle}, #{desired_access.to_s(8)}, #{token_handle})")
  end
  module_function :open_process_token

  def lookup_privilege_value(name, system_name = nil)
    luid = FFI::MemoryPointer.new(API::LUID.size)
    result = API.lookup_privilege_value(WideString.new(system_name.to_s),
      WideString.new(name.to_s), luid)

    return API::LUID.new(luid) unless result == 0
    raise Puppet::Util::Windows::Error.new(
  ...
  def get_token_information(token_handle, token_information)
    ...
    token_information_buffer = FFI::MemoryPointer.new(return_length)
    result = API.get_token_information(token_handle, token_information,
      token_information_buffer, return_length, return_length_ptr)

    if !result
      raise Puppet::Util::Windows::Error.new(
        "GetTokenInformation(#{token_handle}, #{token_information}, #{token_information_buffer}, " +
          "#{return_length}, #{return_length_ptr})")
    end

    raw_privileges = API::Token_Privileges.new(token_information_buffer)
    privileges = { :count => raw_privileges[:privilege_count], :privileges => [] }

    offset = token_information_buffer
    # create a new pointer, offset into Token_Privileges struct by the DWORD count
    offset += API::Token_Privileges.offsets.find { |o| o[0] == :privileges }[1]
  ...
  def process_privilege_symlink?
    begin
      ...
      token_handle = open_process_token(handle, TOKEN_ALL_ACCESS)
      ...
      [handle, token_handle].each do |h|
        API.close_handle(h) if h
      end
    end
  end
